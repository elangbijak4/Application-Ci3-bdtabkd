...
class Model_frommyframework extends CI_Model
{
    ...
    //==========================END TAMBAHAN BARU untuk  general_insertion_model tapi ga pake alert=======================

    //===========================TAMBAHAN BARU 1 ROUND 2=================================    
    public function penarik_isi_tabel_ke_array($table,$kolom_rujukan){
        $this->db->where($kolom_rujukan['nama_kolom'], $kolom_rujukan['nilai']);
        $query =  $this->db->get($table);
        $semua_nama_kolom=$this->penarik_semua_nama_kolom_sebuah_tabel($table);
        //$query   = $builder->get();  // Produces: SELECT * FROM mytable
        $array_tabel=array();
        foreach ($query->result() as $krow=>$row){
            foreach($semua_nama_kolom as $key=>$nama){
                $array_tabel[$krow][$nama]=$row->$nama;
            }
        }
        return $array_tabel;
    }
